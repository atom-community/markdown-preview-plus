...
export function toggle() {
  ...
  if ((needle = editor.getGrammar().scopeName, !grammars.includes(needle))) { return; }

  if (!removePreviewForEditor(editor)) { addPreviewForEditor(editor); }
}
...
export function copyHtml<T>(callback?: (text: string) => T, scaleMath?: number): T
export function copyHtml(callback: (text: string) => any = clipboardCopy, scaleMath = 100): any {
  const editor = atom.workspace.getActiveTextEditor();
  if (editor == null) { return; }

  const text = editor.getSelectedText() || editor.getText();
  const renderLaTeX = atom.config.get('markdown-preview-plus.enableLatexRenderingByDefault');
  return renderer.toHTML(text, editor.getPath(), editor.getGrammar(), renderLaTeX, true, function(error: Error, html: string) {
    if (error) {
